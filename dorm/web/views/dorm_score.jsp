    ...
    <table border="2px">
        ...
        <c:forEach var="dormitory" items="${pageBean.list}">
            <tr>
                ...
                <td>${dormitory.score}</td>
                <td>
                    <a href="${pageContext.request.contextPath}/dorm?m=dormScore&id=${dormitory.id}&currentPage=${pageBean.currentPage}">修改评分</a>
...
<div style="text-align: center">
    ...
    <c:choose>
        ...
        <c:otherwise>
            <a href="${pageContext.request.contextPath}/dorm?m=dormInfo&isScore=true&currentPageNum=1">首页</a>&nbsp;
            <a href="${pageContext.request.contextPath}/dorm?m=dormInfo&isScore=true&currentPageNum=${pageBean.currentPage -1 }">上一页</a>
        </c:otherwise>
    </c:choose>
    &nbsp;
    <%--设置起始页码--%>
    ...
    <c:forEach var="currentPageNum" begin="${start}" end="${last}">
        <c:choose>
            ...
            <c:otherwise>
                <a href="${pageContext.request.contextPath}/dorm?m=dormInfo&isScore=true&currentPageNum=${currentPageNum}">[${currentPageNum}]</a>&nbsp;
            </c:otherwise>
        </c:choose>
    ...
    <c:choose>
        ...
        <c:otherwise>
            <a href="${pageContext.request.contextPath}/dorm?m=dormInfo&isScore=true&currentPageNum=${pageBean.currentPage + 1 }">下一页</a>&nbsp;
            <a href="${pageContext.request.contextPath}/dorm?m=dormInfo&isScore=true&currentPageNum=${pageBean.toatlPageNumber }">尾页</a>
        </c:otherwise>
    </c:choose>
